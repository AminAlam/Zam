...
@click.option('--mahsa_message', default=False, help="A message about Mahsa Amini's murder will be sent to the channel with a timer which is updated evety few seconds.")
...
@click.option('--user_tweet_limit', default=10, type=click.IntRange(1, 120, clamp=True), help="Maximum number of the tweets that an user can send during 1 hour period")
def main(time_diff, mahsa_message, reference_snapshot, num_tweets_to_preserve, user_tweet_limit):
    ...
    suggestions_bot = TelegramSuggestedTweetsBot(creds, twitter_api, db_log, time_diff, reference_snapshot, user_tweet_limit)
